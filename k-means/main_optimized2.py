...
try:
    ...
    if X.shape[0] > 0:
        ...
        if USE_GPU:
            ...
            X_embedded = cuUMAP(n_neighbors=50, min_dist=0.05, metric='cosine').fit_transform(X_gpu)
            X_embedded = cp.asnumpy(X_embedded)
        else:
            print("UMAP CPU activé (UMAP sklearn)")
            X_embedded = umap.UMAP(n_neighbors=50, min_dist=0.05, metric='cosine').fit_transform(X)
