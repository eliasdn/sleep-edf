...
try:
            ...
            try:
                ...
                for task in batch:
                    try:
                        ...
                        results.append([0.0] * (len(bands) + 2 + 6))
    ...
    if X.shape[0] > 0:
        ...
        if USE_GPU:
            print("UMAP GPU optimisé activé")
            X_gpu = cp.asarray(X.astype(np.float32))
            X_embedded = cuUMAP(n_neighbors=n_neighbors_umap, min_dist=0.05, metric='cosine').fit_transform(X_gpu)
            X_embedded = cp.asnumpy(X_embedded)
        else:
            print("UMAP CPU optimisé activé")
            X_embedded = umap.UMAP(n_neighbors=n_neighbors_umap, min_dist=0.05, metric='cosine').fit_transform(X)
